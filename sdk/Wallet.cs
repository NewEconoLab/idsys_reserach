...
namespace sdk
{
    ...
    public class Wallet
    {
        ...
        public Transaction MakeContractTransactionNoSign(string from,string to,decimal sendcount)
        {
            string assetid = id_GAS;
            List<Utxo> utxos = GetUtxo(from, assetid);
            ...
            if (count >= sendcount)//输入大于等于输出
            {
                ...
                //输出
                if (sendcount > decimal.Zero && to.Length > 0)
                {
                    ThinNeo.TransactionOutput output = new ThinNeo.TransactionOutput();
                    output.assetId = new Hash256(assetid);
                    output.value = sendcount;
                    output.toAddress = ThinNeo.Helper_NEO.GetScriptHash_FromAddress(to);
                ...
                if (change > decimal.Zero)
                {
                    ...
                    outputchange.toAddress = ThinNeo.Helper_NEO.GetScriptHash_FromAddress(from);
                    outputchange.value = change;
                    outputchange.assetId = new Hash256(assetid);
